...
export default function PurchaseEdit({ order, suppliers }) {
    const { data, setData, patch, errors, processing } = useForm({
        supplier_id: order.supplier_id || '', po_number: order.po_number || '', status: order.status || 'pending', total_amount: order.total_amount || '', order_date: order.order_date || '', expected_delivery_date: order.expected_delivery_date || '',
    });
    ...
    return (
        <AuthenticatedLayout>
            <Head title={`Edit PO: ${order.po_number}`} />
            <div className="py-12">
